...
const CallScreen: React.FC = () => {
  ...
  const fetchStations = async () => {
    const response = await axios.get(`http://10.0.2.2:8080/robotstations`);
  ...
  const fetchRequests = async (userId: number) => {
    const response = await axios.get(
      `http://10.0.2.2:8080/robot_request/user/${userId}/status_notIn/0,4`,
    );
    if (response.status === 200) {
  ...
  const completeHandler = useCallback(async (request: Robot_Request) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://10.0.2.2:8080/robot_request/${request.request_id}`,
        {
      ...
      if (response.status === 200) {
        ...
        const updateRobotStationResponse = await axios.put(
          `http://10.0.2.2:8080/robot/${request.robot_id}/station/${request.destination_station_id}`
        ...
        const updateSlotsResponse = await axios.put(
          `http://10.0.2.2:8080/update_slots`
        );
