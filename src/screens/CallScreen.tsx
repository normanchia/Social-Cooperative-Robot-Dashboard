...
const CallScreen: React.FC = () => {
  ...
  const fetchStations = async () => {
    const response = await axios.get(`https://itp3111.as.r.appspot.com/robotstations`);
  ...
  const fetchRequests = async (userId: number) => {
    const response = await axios.get(
      `https://itp3111.as.r.appspot.com/robot_request/user/${userId}/status_notIn/0,4`,
    );
    if (response.status === 200) {
  ...
  const completeHandler = useCallback(async (request: Robot_Request) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://itp3111.as.r.appspot.com/robot_request/${request.request_id}`,
        {
      ...
      if (response.status === 200) {
        ...
        const updateRobotStationResponse = await axios.put(
          `https://itp3111.as.r.appspot.com/robot/${request.robot_id}/station/${request.destination_station_id}`
        ...
        const updateSlotsResponse = await axios.put(
          `https://itp3111.as.r.appspot.com/update_slots`
        );
